...
class AgentPPO(AgentBase):
    ...
    def _explore_one_env(self, env, horizon_len: int, if_random: bool = False) -> tuple[TEN, TEN, TEN, TEN, TEN, TEN]:
        ...
        """
        states = th.zeros((horizon_len, self.state_dim[0], self.state_dim[1]), dtype=th.float32).to(self.device)
        ...
        states = states.view((horizon_len, 1, self.state_dim[0], self.state_dim[1]))
        actions = actions.view((horizon_len, 1, self.action_dim)) \
            if not self.if_discrete else actions.view((horizon_len, 1, 1))
...
class ActorPPO(th.nn.Module):
    def __init__(self, net_dims: list[int], state_dim: [int], action_dim: int):
        ...
        self.net = CNN(state_dim, action_dim)
        ...
        self.state_avg = nn.Parameter(th.zeros((state_dim[0],state_dim[1])), requires_grad=False)
        self.state_std = nn.Parameter(th.ones((state_dim[0],state_dim[1])), requires_grad=False)
...
class CNN(nn.Module):
    ...
    参数:
        dim (int): 输入数组的维度 (dim × dim)。
        out_dim (int): 输出的 logit 数量。
        conv_layers (int): 卷积层的数量，默认为 2。
        initial_channels (int): 第一层卷积的输出通道数，默认为 4。
        channel_growth (int): 每层卷积通道数的增长倍数，默认为 2。
    """
    def __init__(self, state_dim: [int], out_dim: int, conv_layers: int = 2, initial_channels: int = 4, channel_growth: int = 2):
        ...
        # 定义输入通道数
        self.input_channels = 1  # 假设输入是单通道的
        
        # 定义卷积层
        self.conv_layers_list = nn.ModuleList()
        output_channels = initial_channels
        for i in range(conv_layers):
            self.conv_layers_list.append(nn.Conv2d(self.input_channels, output_channels, kernel_size=3, stride=1, padding=1))
            self.conv_layers_list.append(nn.ReLU())
            self.conv_layers_list.append(nn.MaxPool2d(kernel_size=2, stride=2))
            self.input_channels = output_channels
            output_channels *= channel_growth  # 每层卷积后通道数翻倍
        
        # 计算经过卷积和池化后的特征图大小
        self.conv_dim = 32
        for _ in range(conv_layers):
            self.conv_dim = self.conv_dim // 2  # 每次池化后尺寸减半
        
        # 定义全连接层
        self.fc1 = nn.Linear(self.input_channels * self.conv_dim * self.conv_dim, 128)
        self.fc2 = nn.Linear(128, out_dim)

    def forward(self, x):
        """前向传播"""
        x = x.view((-1, 1, self.state_dim[0], self.state_dim[1]))
        # 通过卷积层
